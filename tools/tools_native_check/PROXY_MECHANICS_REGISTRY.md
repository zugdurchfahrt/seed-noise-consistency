...
- `named_wrapper_strict` — legacy strict accessor label: строгий descriptor-owner/shape контракт на `descriptorOwner`, но публичный getter surface materializes as `synthetic_named` через именованный getter + `markAsNative` (без public Proxy).
- `strict_accessor_gateway` — текущий strict scalar accessor gateway: строгий descriptor-owner/shape контракт для scalar accessors; публичный getter surface materializes as `synthetic_named` через `__wrapStrictAccessor`.
- `object_return_gateway` — текущий object-return accessor gateway: строгий descriptor-owner/shape контракт для object/collection-return accessors; bucket отделён от scalar gateway, публичный getter surface materializes as `synthetic_named` через `__wrapStrictAccessor`.
...
- `core_proxy` ⇔ `wrapLayer=core_wrapper` и прямое использование `__wrapNative*`.
...
1. `Core.applyTargets(...)` с явным `wrapLayer` из списка:
   ...
   * `named_wrapper_strict` (legacy accessor strict, synthetic_named),
   * `strict_accessor_gateway` (strict scalar accessor gateway, synthetic_named),
   * `object_return_gateway` (object-return accessor gateway, synthetic_named),
...
- `Core.applyTargets(kind=accessor, wrapLayer in {'named_wrapper_strict','strict_accessor_gateway','object_return_gateway'}, policy='strict')` [wrapperClass=synthetic_named] --- accessor gateway contract: preflight обязан найти `origDesc` и `descriptorOwner` по proto-chain; `defineProperty` делается только на `descriptorOwner`; shape change `accessor -> data` разрешён только для strict scalar gateway при явном `allowShapeChange=true`; `enumerable/configurable` берутся из `origDesc`; `set` переносится из `origDesc` как есть (включая `undefined/null`); `get=wrappedGet`.
- `wrappedGet` для accessor gateway [wrapperClass=synthetic_named] --- getter materializes as named synthetic accessor (`__wrapStrictAccessor` для unified gateway, named getter + `markAsNative` для legacy `named_wrapper_strict`); invalid-this путь делегируется в native getter через `Reflect.apply`, public getter surface не должен оставаться observable как Proxy-anonymous function.
...
- `kind=accessor` и нужен строгий descriptor-owner/shape контракт ⇒ `policy='strict'` + один из accessor gateway labels:
  `strict_accessor_gateway` для strict scalar accessors,
  `object_return_gateway` для object-return identity surfaces,
  `named_wrapper_strict` только как legacy-compatible strict accessor label.
...
- `window.Core.applyTargets` (`core_window.js`) [wrapperClass=engine] --- declarative descriptor-plan engine (`accessor|method|promise_method|data`) with preflight + plan + rollback-info (apply/rollback делает вызывающий код); materializes `wrapLayer`/`invokeClass`/`policy` in returned plan-items (contract trace), включая accessor gateway contract (`named_wrapper_strict` legacy, `strict_accessor_gateway`, `object_return_gateway`, `allowShapeChange` only for strict scalar gateway).
...
- `Navigator.webdriver` (`hide_webdriver.js`) [wrapperClass=synthetic_named] --- `strict scalar accessors gateway`: target-contract через `Core.applyTargets(kind=accessor, wrapLayer='strict_accessor_gateway', policy='strict')`; receiver contract держится на owner/shape-stable accessor path, public getter surface stays named/native-shaped without public Proxy.
...
- `Navigator.deviceMemory/hardwareConcurrency/language/languages` (`nav_total_set.js`) [wrapperClass=synthetic_named] --- `strict scalar accessors gateway`: target-contract via `Core.applyTargets` with `wrapLayer='strict_accessor_gateway'` + `policy='strict'` (owner/flags shape-stability); group-level policy fixed as `throw` (без downgrade в `skip`).

- `NavigatorUAData.brands/mobile/platform` (`nav_total_set.js`) [wrapperClass=mixed] --- `userAgentData.*` primitive patches via `Core.applyTargets` (`descriptor_only` for data-shape, otherwise strict scalar accessor path via `wrapLayer='strict_accessor_gateway'` + `policy='strict'`).
...
- `Navigator.plugins/mimeTypes/userAgentData` (`nav_total_set.js`) [wrapperClass=synthetic_named] --- `object-return identity surfaces gateway`: accessor patch via `Core.applyTargets(kind=accessor, wrapLayer='object_return_gateway', policy='strict')`; object-return bucket отделён от strict scalar bucket на уровне label/trace, при этом paired callable surfaces follow method gateway rules.
