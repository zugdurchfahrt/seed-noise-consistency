...
Для этого достаточно в def main(): закомментировать вызовы методов экземпляра VPNClient, отвечающих за проверку, подготовку и подключение VPN: 
        ...
        #  client.connect()
        client.post()
В таком режиме скрипт будет работать точно так же, как и раньше: все последующие шаги выполняются, только без остановки/запуска OpenVPN-процессов. Если у вас уже поднят VPN любым другим способом, скрипт просто будет использовать текущее сетевое окружение.
