...
function printToStringCrossRealmChecks() {
  ...
  function printLastDegradeEvents() {
    ...
    const rows = last50.map((e, i) => ({
      ...
      message: e && e.extra && e.extra.message ? e.extra.message : null,
      data: (() => {
        try { return JSON.stringify(e?.extra?.data); }
  ...
  window.__PROBE_OUTPUT__ = result;
...
__probeDownloadHtmlReport(window.__PROBE_OUTPUT__);
...
// Вставь в DevTools Console на целевой странице.
