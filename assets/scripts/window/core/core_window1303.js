  ...
  (function installCoreApplyTargets(){
    ...
    try {
      ...
      function wrapGetter(key, getter, desc, validThis, opts) {
        ...
        if (wrapLayer === 'named_wrapper_strict' && typeof origGet === 'function') {
          const wrapped = __wrapStrictAccessor(key, getter, desc, validThis);
          knownWrapped.add(wrapped);
          return wrapped;
        }
      ...
      function patchAccessor(planItem, t, desc, markAsNative, fail) {
        ...
        if (strictAccessorContract && typeof getWrapped === 'function') {
          planItem.wrapperClass = (typeof toStringProxyTargetMap.get(getWrapped) === 'function')
            ? 'core_proxy'
            : 'synthetic_named';
        }
