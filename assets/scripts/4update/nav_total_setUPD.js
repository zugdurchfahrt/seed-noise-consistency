const NavTotalSetPatchModule = function NavTotalSetPatchModule(window) {
  if (!window.__PATCH_NAVTOTALSET__) {
    ...
    // One-time degrade warnings (helps catch race/init issues without crashing callers)
    const __NAV_WARNED__ = (G.__NAV_PATCH_WARNED__ instanceof Set)
      ? G.__NAV_PATCH_WARNED__
      : (G.__NAV_PATCH_WARNED__ = new Set());
    const warnOnce = (id, ...args) => {
      if (__NAV_WARNED__.has(id)) return;
      __NAV_WARNED__.add(id);
    ...
    if ('userAgentData' in navigator) {
        ...
        brands: {
          get: mark(function getBrands(){
            if (!Array.isArray(meta.brands) || !meta.brands.length) {
              warnOnce('uad.brands.missing', '[nav_total_set] uaData.brands missing -> fallback to native');
              return (dBrands && typeof dBrands.get === 'function') ? dBrands.get.call(this) : undefined;
            }
            return meta.brands;
          }, 'get brands'),
          enumerable: true,
        ...
        mobile: {
          get: mark(function getMobile(){
            if (typeof meta.mobile !== 'boolean') {
              warnOnce('uad.mobile.missing', '[nav_total_set] uaData.mobile missing -> fallback to native');
              return (dMobile && typeof dMobile.get === 'function') ? dMobile.get.call(this) : undefined;
            }
            return meta.mobile;
          }, 'get mobile'),
          enumerable: true,
        ...
        platform: {
          get: mark(function getPlatform(){
            if (typeof chPlatform !== 'string' || !chPlatform) {
              warnOnce('uad.platform.missing', '[nav_total_set] uaData.platform missing -> fallback to native');
              return (dPlatform && typeof dPlatform.get === 'function') ? dPlatform.get.call(this) : undefined;
            }
            return chPlatform;
          }, 'get platform'),
          enumerable: true,
      ...
      const nativeFullGet = (Object.getOwnPropertyDescriptor(uadProto, 'fullVersionList')
      ...
      const getFullVersionList = mark(function getFullVersionList(){
        if (!Array.isArray(meta.fullVersionList) || !meta.fullVersionList.length) {
          warnOnce('uad.fullVersionList.missing', '[nav_total_set] uaData.fullVersionList missing -> fallback to native');
          return (typeof nativeFullGet === 'function') ? nativeFullGet.call(this) : undefined;
        }
      ...
      const nativeUaFullGet = (Object.getOwnPropertyDescriptor(uadProto, 'uaFullVersion')
      ...
      const getUaFullVersion = mark(function getUaFullVersion(){
        if (typeof meta.uaFullVersion !== 'string' || !meta.uaFullVersion) {
          warnOnce('uad.uaFullVersion.missing', '[nav_total_set] uaData.uaFullVersion missing -> fallback to native');
          return (typeof nativeUaFullGet === 'function') ? nativeUaFullGet.call(this) : undefined;
        }
      ...
      const getHighEntropyValues = mark(function getHighEntropyValues(keys) {
          ...
          for (const hint of keys) {
            ...
            if (!(hint in map)) {
              warnOnce('uad.ghev.missing', '[nav_total_set] uaData.getHighEntropyValues missing fields -> fallback to native');
              const orig = ghevDesc && ghevDesc.value;
              if (typeof orig === 'function') return orig.call(this, keys);
              throw new Error(`THW: missing highEntropy.${hint}`);
            }
            const val = map[hint];
            if (val === undefined || val === null
              || (typeof val === 'string' && !val && hint !== 'model')
