      ...
      function invoke(self, argsLike) {
          ...
          try {
              ...
              for (const hook of hooks) {
                  ...
                  // If hook calls orig itself, prevent second orig call (void methods like readPixels).
                  let called = false;
                  let calledResult;

                  const origOnce = function(...a) {
                      called = true;
                      calledResult = orig.apply(self, a);
                      return calledResult;
                  };

                  try {
                      const res = hook.apply(self, [origOnce, ...patched]);

                      // If hook already invoked orig:
                      if (called) {
                          if (Array.isArray(res)) {
                              throw new Error(`[patchMethod] hook "${hook.name || 'anon'}" called orig and returned args for "${method}"`);
                          }
                          if (res !== undefined) {
                          ...
                          return calledResult; // usually undefined for void methods
                      }

                      // result substitution (hook did not call orig)
